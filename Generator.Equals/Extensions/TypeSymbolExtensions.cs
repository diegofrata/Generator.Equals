...
static class TypeSymbolExtensions
{
    ...
    /// <summary>
    /// Checks if a type's equality was generated by Generator.Equals, either via a visible
    /// [Equatable] attribute (same assembly) or via the generated nested EqualityComparer class
    /// (cross-assembly, where the [Conditional] attribute has been erased).
    /// Unlike <see cref="HasProperEquality"/>, this does not match types that merely implement
    /// IEquatable&lt;T&gt;, which may still use reference semantics (e.g. ImmutableArray&lt;T&gt;).
    /// </summary>
    public static bool HasGeneratedEquality(this ITypeSymbol typeSymbol)
    {
        if (typeSymbol.HasEquatableAttribute())
            return true;

        return typeSymbol is INamedTypeSymbol namedType && namedType.HasGeneratedEqualityComparer();
    }
    ...
    /// </summary>
    public static bool HasGeneratedEqualityComparer(this INamedTypeSymbol typeSymbol)
    {
        var comparerType = typeSymbol.GetTypeMembers("EqualityComparer").FirstOrDefault();
        if (comparerType is null || !comparerType.IsSealed)
            return false;

        // Check implements IEqualityComparer<T>
        return comparerType.Interfaces.Any(i =>
